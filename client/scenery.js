...
function sky(ctx,w,h){
  const g=ctx.createLinearGradient(0,0,w,h);g.addColorStop(0,'#b8b7e3');g.addColorStop(.42,'#d5c3e5');g.addColorStop(1,'#a9cddf');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);
  for(let i=0;i<8;i++){
...
export function drawGarden(ctx,map){
  ctx.drawImage(cached(map,c=>{
    sky(c,map.width,map.height);ellipse(c,590,475,445,205,'#b0c9d6');ellipse(c,590,450,445,205,'#e1f4eb');ellipse(c,590,445,406,179,'#cce8e0');
    ...
    ellipse(c,600,165,62,62,'#fff2cc');ellipse(c,627,148,51,51,'#c9c7e6');
    c.font='28px "Jua","Malgun Gothic",sans-serif';c.textAlign='center';c.fillStyle='#658a88';c.fillText('달구름 정원',600,640);
    c.font='17px "Jua","Malgun Gothic",sans-serif';c.fillText('구름처럼 가볍게, 친구와 쉬어가요',600,676);
  }),0,0);
}
