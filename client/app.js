...
let selfId=null,room=null,busy=false,toastTimer,mode='student',held=new Set(),touch={x:0,y:0},last={x:0,y:0};
...
if(fragment.has('teacher')){$('teacher-key').value=fragment.get('teacher');setMode('teacher');history.replaceState(null,'',location.pathname);}
...
function updateRoom(value){
  ...
  $('players').replaceChildren(...room.players.map(p=>{
    ...
    const state=document.createElement('span');state.textContent=!p.connected?'다시 연결 중':p.role==='teacher'?'선생님':'LV 1';
...
function enter(result){
  ...
  $('room-badge').hidden=false;$('leave').hidden=false;$('touch-controls').hidden=false;
...
function reset(message){
  ...
  $('lobby').hidden=false;$('room-badge').hidden=true;$('leave').hidden=true;$('touch-controls').hidden=true;
