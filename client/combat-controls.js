...
// Q의 대상·피해는 서버가 결정합니다. E의 전투 스킬은 아직 준비 중입니다.
export function createCombatControls({getPlayer,canAct,toast,request}){
  ...
  async function act(kind){
    ...
    if(kind==='skill'){
      try{await request('combat:skill',{});toast('전투 스킬은 준비 중이에요. 지금은 별자리 이펙트를 미리 볼 수 있어요.');}catch(error){toast(error.message);}return;
    }
