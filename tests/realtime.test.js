...
test('위쪽 세 맵은 가까운 연결 문으로만 왕복하고 건너뛰기 요청을 거절한다',async t=>{
 const {connect,game}=await fixture(t),teacher=await connect(),r=await create(teacher),s=await connect();
 const j=await call(s,'room:join',{code:r.room.code,nickname:'1'});assert.ok(j.ok);
 const p=game.store.rooms.get(r.room.code).players.get(j.selfId);
 assert.equal((await call(s,'map:travel',{to:ORIGIN_MAPS[2].id})).ok,false);
 assert.equal((await call(s,'map:travel',{to:ORIGIN_MAPS[0].id})).ok,false);
 for(const to of [...ORIGIN_MAPS.map(m=>m.id),ORIGIN_MAPS[1].id,ORIGIN_MAPS[0].id,PLAZA_ID]){
   const gate=mapOf(p.mapId).objects.find(o=>o.target===to);assert.ok(gate);
   p.x=gate.x;p.y=gate.y;
   assert.equal((await call(s,'map:travel',{to})).ok,true);assert.equal(p.mapId,to);
   const map=mapOf(to);assert.ok(p.x>0&&p.x<map.width&&p.y>0&&p.y<map.height);
 }
});
...
test('chat text is validated before anything else, independent of cooldown',async t=>{
 const {connect}=await fixture(t),teacher=await connect(),r=await create(teacher);
 const s=await connect();await call(s,'room:join',{code:r.room.code,nickname:'1'});
 for(const text of ['','   ','a'.repeat(101),'hi'+String.fromCharCode(7)+'there','줄'+String.fromCharCode(10)+'바꿈',123]){
  const res=await call(s,'chat:send',{text});
  assert.equal(res.ok,false);assert.equal(res.error,'채팅은 1~100자로 입력해주세요.');
 }
});
...
test('planet rules require membership, interior board proximity, and expectedRules concurrency guard',async t=>{
 ...
 student.avatar.departmentId=reading.planetId;student.mapId=interiorIdOf(reading.planetId);student.x=board.x;student.y=board.y;
 const states=[];teacher.on('room:state',st=>states.push(st));
 const sysMsgs=[];teacher.on('chat:message',m=>sysMsgs.push(m));
 const opened=await call(s,'planet:rules:open',{planetId:reading.planetId});
 assert.equal(opened.ok,true);const initialRules=structuredClone(opened.rules);
 assert.deepEqual(opened,{ok:true,rules:initialRules,planetId:reading.planetId,name:'독서행성'});
 const newRules=['첫째 줄','둘째 줄'];
 const ok=await call(s,'planet:rules:set',{planetId:reading.planetId,rules:newRules,expectedRules:initialRules});
 assert.equal(ok.ok,true);
 await sleep(30);
 assert.ok(states.some(st=>JSON.stringify(st.planets.find(pl=>pl.id===reading.planetId).rules)===JSON.stringify(newRules)));
 assert.ok(sysMsgs.some(m=>m.text==='1 친구가 독서행성의 규칙을 바꿨어요.'));
 const stale=await call(s,'planet:rules:set',{planetId:reading.planetId,rules:['오래된 수정'],expectedRules:[]});
 assert.equal(stale.ok,false);assert.equal(stale.error,'다른 친구가 규칙을 바꿨어요. 창을 닫고 다시 열어 확인해주세요.');
 assert.deepEqual((await call(s,'planet:rules:open',{planetId:reading.planetId})).rules,newRules);
 student.avatar.departmentId=null;
 assert.equal((await call(s,'planet:rules:open',{planetId:reading.planetId})).ok,false);
 student.avatar.departmentId=reading.planetId;student.mapId=PLAZA_ID;
 assert.equal((await call(s,'planet:rules:open',{planetId:reading.planetId})).ok,false);
 student.mapId=interiorIdOf(reading.planetId);student.x=board.x+1000;student.y=board.y;
 assert.equal((await call(s,'planet:rules:open',{planetId:reading.planetId})).ok,false);
 const teacherPlayer=[...room.players.values()].find(p=>p.role==='teacher');teacherPlayer.mapId=interiorIdOf(reading.planetId);teacherPlayer.x=board.x;teacherPlayer.y=board.y;
 assert.equal((await call(teacher,'planet:rules:set',{planetId:reading.planetId,rules:['교사 수정'],expectedRules:newRules})).ok,true);
 assert.ok(sysMsgs.some(m=>m.text==='선생님이 독서행성의 규칙을 바꿨어요.'));
 const tooMany=await call(teacher,'planet:rules:set',{planetId:reading.planetId,rules:Array.from({length:9},(_,i)=>'줄'+i),expectedRules:['교사 수정']});
 assert.equal(tooMany.ok,false);assert.equal(tooMany.error,'규칙은 1~8줄, 한 줄 40자 이내로 적어주세요.');
 const tooLong=await call(teacher,'planet:rules:set',{planetId:reading.planetId,rules:['a'.repeat(41)],expectedRules:['교사 수정']});
 assert.equal(tooLong.ok,false);
 const control=await call(teacher,'planet:rules:set',{planetId:reading.planetId,rules:['줄'+String.fromCharCode(7)+'바꿈'],expectedRules:['교사 수정']});
 assert.equal(control.ok,false);
 const empty=await call(teacher,'planet:rules:set',{planetId:reading.planetId,rules:[],expectedRules:['교사 수정']});
