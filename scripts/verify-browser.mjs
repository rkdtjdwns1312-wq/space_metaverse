...
import { PLAZA_ID, STREET_ID, STREET, MAP, INTERIOR, SHOP, BAG, PLANET_TEMPLATES, RULES } from '../shared/config.js';
...
async function worldClick(page,mx,my){
  ...
  await page.mouse.click(box.x+(mx-view.x)*view.scale,box.y+(my-view.y)*view.scale);
}
...
async function openSocialFromDock(page){await closeOpenDialogs(page);await page.locator('#dock-chat').click();await page.locator('#social-dialog').waitFor({state:'visible'});}
async function openInventoryFromDock(page){await closeOpenDialogs(page);await page.locator('#dock-inventory').click();await page.locator('#inventory-dialog').waitFor({state:'visible'});}
async function openAvatarFromDock(page){await closeOpenDialogs(page);await page.locator('#dock-avatar').click();await page.locator('#avatar-dialog').waitFor({state:'visible'});}
async function clickMenuAction(page,id){await openMenuFromDock(page);await page.locator('#'+id).click();}
async function openMenuFromDock(page){await closeOpenDialogs(page);await page.locator('#dock-menu').click();await page.locator('#menu-dialog').waitFor({state:'visible'});}
try{
 const teacherContext=await newContext({viewport:{width:1440,height:1000}});
 const studentContext=await newContext({viewport:{width:1440,height:1000},hasTouch:true});
 ...
 await teacherHello.waitFor({state:'attached'});
 ...
 await maskedEntry.waitFor({state:'attached'});
 await maskedEntry.locator('.badge').filter({hasText:'순화됨'}).waitFor({state:'attached'});
 ...
 await teacher.locator('#chat-log li').filter({hasText:'1 친구의 채팅이 잠시 멈췄어요.'}).waitFor({state:'attached'});
 await closeOpenDialogs(teacher);await teacher.locator('#dock-chat').click();await teacher.locator('#social-dialog').waitFor({state:'visible'});await openSocialFromDock(teacher);await teacher.locator('#crew-button').click();await teacher.locator('#crew-dialog').waitFor({state:'visible'});
 await muteButton.filter({hasText:'허용'}).waitFor({state:'attached'});
 await muteButton.click();
 await waitForChatInput(student,{disabled:false,placeholder:'친구들에게 말해요 (Enter)'});
 await muteButton.filter({hasText:'금지'}).waitFor({state:'attached'});
 check('Teacher mute toggle disables/enables one student\'s chat input and announces it by nickname');
 ...
 await student.locator('#chat-input').press('Enter');
 await teacher.locator('#chat-log li').filter({hasText:'말풍선 테스트'}).waitFor({state:'attached'});
 ...
 check('Sending a chat message shows a speech bubble that changes the teacher canvas');

 assert.ok(await student.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 check('390px layout with the populated chat panel still has no horizontal overflow');
 ...
 check('Student proposes a new planet "급식행성" by placing it on the map, picking a planet type, and filling the create dialog');
 ...
 await teacher.locator('#proposals li').filter({hasText:'급식행성'}).filter({hasText:'신청: 1'}).waitFor({state:'attached'});
 check('Teacher opens "선생님 도구" and sees the pending proposal with the proposing student\'s nickname');
 ...
 await student.locator('#planet-create-submit').click();
 await student.locator('#planet-create-error').filter({hasText:'이미 승인을 기다리는 행성이 있어요.'}).waitFor({state:'attached'});
 check('A student with one pending proposal cannot submit a second one');
 ...
 await student.locator('#self-department').filter({hasText:'소속: 🍱 급식행성'}).waitFor({state:'attached'});
 ...
 check('Teacher approves the proposal; the room gains exactly one planet named 급식행성');
 await student.locator('#chat-log li').filter({hasText:'첫 멤버'}).waitFor({state:'attached'});
 check('Student chat log shows the approval announcement mentioning the first member');
 await teacher.locator('#proposals-empty').waitFor({state:'visible'});
 check('Teacher proposals list shows the empty state once no proposals remain');
 ...
 await student.locator('#planet-member-count').filter({hasText:'1'}).waitFor({state:'attached'});
 check('E key opens the planet dialog for a member: enter shown, join hidden, leave shown, member count 1');
 await student.locator('#planet-rules-toggle').click();
 await student.locator('#planet-rules-list li').first().waitFor({state:'attached'});
 ...
 check('Planet rules list starts with the chosen type\'s default rules');
 ...
 await student.waitForFunction(id=>!document.getElementById(id).hidden,'planet-exit');
 await student.locator('#map-caption').filter({hasText:'급식행성 안'}).waitFor({state:'attached'});
 await openSocialFromDock(teacher);await teacher.locator('#crew-button').click();
 await teacher.locator('#crew-dialog').waitFor({state:'visible'});
 await teacher.locator('#players li').filter({hasText:'급식행성 안'}).waitFor({state:'attached'});
 await teacher.locator('#crew-close').click();
 await teacher.locator('#crew-dialog').waitFor({state:'hidden'});
 ...
 await student.waitForFunction(id=>!document.getElementById(id).hidden,'planet-info');
 await clickMenuAction(student,'planet-info');
 await student.locator('#planet-dialog').waitFor({state:'visible'});
 await student.locator('#planet-enter').waitFor({state:'hidden'});
 check('#planet-info inside the planet opens its dialog with the enter button hidden');
 await student.locator('#planet-rename-input').fill('급식별');
 await student.locator('#planet-rename-propose').click();
 await student.locator('#planet-title').filter({hasText:'급식별'}).waitFor({state:'attached'});
 check('A sole member\'s rename proposal passes immediately');
 await student.locator('#chat-log li').filter({hasText:'"급식행성" 행성의 이름이 "급식별"로 바뀌었어요!'}).waitFor({state:'attached'});
 check('Chat log announces the automatic single-member rename');
 ...
 const student2Context=await newContext({viewport:{width:1440,height:1000}});
 ...
 await student2.locator('#planet-join').filter({hasText:'가입 승인 대기 중'}).waitFor();
 await closeOpenDialogs(student);
 const mailbox=INTERIOR.objects.find(o=>o.kind==='mailbox');
 const priorPosition={mapId:p.mapId,x:p.x,y:p.y};Object.assign(p,{mapId:'planet:'+cafeteriaId,x:mailbox.x,y:mailbox.y+60});
 game.io.to(p.socketId).emit('room:state',game.store.snapshot(room,p));
 await student.locator('#interact-object').filter({hasText:'가입 신청 우체통'}).waitFor();await student.locator('#touch-interact').click();
 await student.locator('#mailbox-requests li').filter({hasText:'2'}).getByRole('button',{name:'가입 승인',exact:true}).click();
 await student.locator('#mailbox-close').click();Object.assign(p,priorPosition);game.io.to(p.socketId).emit('room:state',game.store.snapshot(room,p));
 await student2.locator('#self-department').filter({hasText:'소속: 🍱 급식별'}).waitFor({state:'attached'});
 check('Second student applies from planet info and an existing member approves at the interior mailbox');
 assert.equal([...room.players.values()].filter(pl=>pl.avatar.departmentId===cafeteriaId).length,2);
 check('Server room state now counts two members for the planet');
 await student2.locator('#planet-rename-input').fill('급식나라');
 await student2.locator('#planet-rename-propose').click();
 await student2.locator('#planet-rename-status').filter({hasText:'찬성 1'}).waitFor({state:'attached'});
 await student2.locator('#planet-rename-status').filter({hasText:'2명'}).waitFor({state:'attached'});
 check('Second student proposes a rename that now needs a majority of 2 members');
 await clickMenuAction(student,'planet-info');
 await student.locator('#planet-dialog').waitFor({state:'visible'});
 await student.locator('#planet-vote-no').click();
 await student.locator('#chat-log li').filter({hasText:'"급식나라" 이름 바꾸기가 부결되었어요.'}).waitFor({state:'attached'});
 await student2.locator('#chat-log li').filter({hasText:'"급식나라" 이름 바꾸기가 부결되었어요.'}).waitFor({state:'attached'});
 check('A member voting no defeats the 2-member rename proposal; both members see the rejection announcement');
 await student2.locator('#planet-rename-input').fill('급식나라');
 await student2.locator('#planet-rename-propose').click();
 await student2.locator('#planet-rename-status').filter({hasText:'찬성 1'}).waitFor({state:'attached'});
 await student.locator('#planet-vote-yes').click();
 await student.locator('#planet-title').filter({hasText:'급식나라'}).waitFor({state:'attached'});
 await student.locator('#chat-log li').filter({hasText:'"급식별" 행성의 이름이 "급식나라"로 바뀌었어요!'}).waitFor({state:'attached'});
 check('Unanimous agreement passes the rename; both members\' dialogs and chat logs reflect the new name');
 ...
 await clickMenuAction(student,'planet-exit');
 ...
 check('Student exits the planet back to the plaza, landing below it');
 ...
 await openSocialFromDock(student);await student.locator('#crew-button').click();
 await student.locator('#crew-dialog').waitFor({state:'visible'});
 assert.ok(await student.locator('#players li').count()>=2);
 check('New layout: #crew-button opens #crew-dialog listing the teacher and the students');
 ...
 check('The map canvas fills the whole viewport; the bag opens from the bottom dock');
 ...
 await student.locator('#chat-log li.private .private-badge').filter({hasText:'나에게만'}).first().waitFor({state:'attached'});
 check('Teacher gives 20 star shards to student 1 by id; the student (not the public chat) gets a private "나에게만" whisper naming the amount');
 ...
 check('Teacher canvas redraws once the student leaves the plaza for star-street');
 await openSocialFromDock(teacher);await teacher.locator('#crew-button').click();
 await teacher.locator('#crew-dialog').waitFor({state:'visible'});
 assert.equal((await teacher.locator('#crew-count').innerText()).trim(),rosterBeforeTravel);
 await teacher.locator('#crew-close').click();
 await teacher.locator('#crew-dialog').waitFor({state:'hidden'});
 check('Teacher crew roster count ('+rosterBeforeTravel+') is unchanged while the student is away on another map');
 await student.waitForTimeout(150);
 ...
 check('Student canvas redraws to the star-street scene after travel');
 ...
 await student.waitForFunction(id=>!document.getElementById(id).hidden,'planet-new');
 check('Walking back through the street gate returns the student to the plaza near its arrival point, with #planet-new visible again');
 ...
 await student.setViewportSize({width:390,height:844});
 {
   ...
   await openAvatarFromDock(student);
   const passportBox=await student.locator('.passport').boundingBox();
   assert.ok(chatBox.width<=390&&passportBox.width<=390,'each dock panel fits at 390px');
 }
 assert.ok(await student.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await openSocialFromDock(student);assert.ok(await student.locator('#crew-button').isVisible());
 ...
 await clickMenuAction(teacher,'teacher-tools');
 await teacher.locator('#teacher-dialog').waitFor({state:'visible'});
 await teacher.locator('#proposals li').filter({hasText:'놀이행성'}).waitFor({state:'attached'});
 await teacher.locator('#proposals li').filter({hasText:'놀이행성'}).locator('button.reject').click();
 await teacher.locator('#proposals-empty').waitFor({state:'visible'});
 await teacher.locator('#teacher-close').click();
 await teacher.locator('#teacher-dialog').waitFor({state:'hidden'});
 await student2.locator('#chat-log li').filter({hasText:'돌려보냈어요'}).waitFor({state:'attached'});
 ...
 await student.locator('#self-department').filter({hasText:'아직 소속 행성이 없어요'}).waitFor({state:'attached'});
 await student2.locator('#self-department').filter({hasText:'아직 소속 행성이 없어요'}).waitFor({state:'attached'});
 await student.locator('#chat-log li').filter({hasText:'없앴어요'}).waitFor({state:'attached'});
 check('Teacher removes the planet; both students lose membership and see the removal announcement');

 // Item 13: 390px layout still holds up after all this planet activity.
 await student.setViewportSize({width:390,height:844});
 await student.waitForFunction(id=>!document.getElementById(id).hidden,'planet-new');
 assert.ok(await student.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 check('#planet-new is visible with no horizontal overflow at 390px after the planet scenario');
 ...
 await clickMenuAction(teacher,'teacher-tools');
 await teacher.locator('#teacher-dialog').waitFor({state:'visible'});
 await teacher.locator('#proposals li').filter({hasText:'손가락행성'}).waitFor({state:'attached'});
 ...
 check('Teacher creates two more planets directly from #planet-new with different templates (pe, art); the teacher canvas redraws after each creation');
 ...
 await teacher.locator('#planet-dialog').waitFor({state:'visible'});
 await teacher.locator('#planet-title').filter({hasText:'⚽'}).waitFor({state:'attached'});
 check('Opening a template-created planet shows its template icon (⚽ for the pe template) in the #planet-title dialog heading');
 await teacher.locator('#planet-close').click();
 await teacher.locator('#planet-dialog').waitFor({state:'hidden'});

 // Scenario 2: avatar card and bag grid (student 1, reset to a normal desktop viewport since Item 14
 // left it at 390px).
 await openAvatarFromDock(student);assert.ok(await student.locator('#avatar-card').isVisible());
 ...
 check('Avatar card shows LV 1 with a non-blank portrait; the bag grid always has '+(BAG.columns*BAG.rows)+' slots and exactly '+heldKindCount+' filled li slot(s) matching the held item kinds');
 ...
 check('Teacher\'s crew dialog shows the star-shards badge on both students\' rows');
 await teacher.locator('#crew-close').click();
 await teacher.locator('#crew-dialog').waitFor({state:'hidden'});
 ...
 await student.locator('#self-effects li').filter({hasText:'반짝반짝'}).waitFor({state:'attached'});
 await student.locator('#chat-log li').filter({hasText:'1 친구가 반짝 별 스티커를 썼어요.'}).waitFor({state:'attached'});
 await student.waitForTimeout(150);
 ...
 await clickMenuAction(teacher,'teacher-tools');
 await teacher.locator('#teacher-dialog').waitFor({state:'visible'});
 await teacher.locator('#item-log li').filter({hasText:'반짝 별 스티커'}).waitFor({state:'attached'});
 await teacher.locator('#teacher-close').click();
 await teacher.locator('#teacher-dialog').waitFor({state:'hidden'});
 ...
 await student.locator('#use-confirm').click();
 await student.locator('#use-dialog').waitFor({state:'hidden'});
 await student2.locator('#chat-log li').filter({hasText:'누군가 2 친구에게 우주 간식을 썼어요.'}).waitFor({state:'attached'});
 ...
 await student.locator('#draw-spread .draw-deck').waitFor({state:'visible'});
 ...
 await student.locator('#bag-list .slot-btn[aria-label="달토끼 × 1"]').click();
 await student.locator('#bag-detail .use').click();
 await student.locator('#draw-spread .draw-deck').click();
 ...
 check('달토끼 단일 카드 더미: 열기·닫기·재접속은 소모 없음, 더미 클릭 한 번으로 카드1개 소모와 무작위 보상 지급');
 ...
 await openAvatarFromDock(student);assert.ok(await student.locator('#avatar-card').isVisible());
 check('At 390px after all the item/trade scenarios, the bag grid causes no horizontal overflow and #avatar-card stays visible');
 ...
 await clickMenuAction(student2,'leave');
 ...
 const teacher2Context=await newContext({viewport:{width:1440,height:1000}});
 ...
 check('Second teacher creates an isolated classroom with the same teacher key');
 ...
 check('Second classroom seeded with #seed-planets has 4 example planets including 독서행성, isolated from the first');
 const intruderContext=await newContext({viewport:{width:1440,height:1000}});
 ...
 await openMenuFromDock(teacher2);await teacher2.getByRole('button',{name:'교실 종료하기'}).click();await teacher2.locator('#confirm-leave').click();
 ...
 errors.push(error.message);writeFileSync('.local/browser-report.json',JSON.stringify({checks,errors},null,2));
 for(const [i,context] of browser.contexts().entries()){const page=context.pages()[0];if(page)await page.screenshot({path:'.local/browser-failure-'+i+'.png',fullPage:true}).catch(()=>{});}
 throw error;
