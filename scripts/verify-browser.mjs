...
try{
 ...
 await teacher.locator('#player-count').filter({hasText:'2 / 30'}).waitFor();check('Allowlist enforced, same-room student appears');
 ...
 check('The map canvas spans the same full width as the .hud row below it (new full-width layout)');
 ...
 check('At 390px the .hud stacks vertically (chat below passport) with no horizontal overflow and #crew-button stays visible');
