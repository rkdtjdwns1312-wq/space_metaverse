...
try{
 ...
 await page.addInitScript(()=>{const ellipse=CanvasRenderingContext2D.prototype.ellipse;window.avatarRingCount=0;CanvasRenderingContext2D.prototype.ellipse=function(x,y,rx,ry,...args){if(this.strokeStyle==='#ffe8a3'||(this.canvas.id==='world'&&rx===23&&ry===8&&this.strokeStyle==='#8061b0'))window.avatarRingCount++;return ellipse.call(this,x,y,rx,ry,...args);};const old=CanvasRenderingContext2D.prototype.drawImage;window.spriteTransforms={};CanvasRenderingContext2D.prototype.drawImage=function(image,...args){if(this.canvas.id==='world'&&image?.src)window.spriteTransforms[new URL(image.src).pathname]=this.getTransform().a;return old.call(this,image,...args);};});
 ...
 for(const c of CONSTELLATIONS)for(const level of [2,3,4,5])for(const sign of [-1,1]){
  ...
  const expectedSign=sign*(((level===2&&LV2_LEFT_FACING.has(c.id))||(level===3&&LV3_LEFT_FACING.has(c.id))||(level===4&&LV4_LEFT_FACING.has(c.id))||(level===5&&LV5_LEFT_FACING.has(c.id)))?-1:1);
  await page.evaluate(path=>{delete window.spriteTransforms[path];window.avatarRingCount=0;},sprite);publish();
  await page.waitForFunction(({sprite,expectedSign})=>Math.sign(window.spriteTransforms[sprite])===expectedSign,{sprite,expectedSign});
  assert.equal(await page.evaluate(()=>window.avatarRingCount),0,`${c.id} LV${level} ring overlay absent`);
 }
 console.log('PASS: all 16 constellations × LV2-5 × left/right: actual canvas drawImage transform matches independent LV2/LV3/LV4/LV5 golden sets');
 ...
 await page.locator('#dock-avatar').click();await page.waitForTimeout(200);assert.equal(await page.evaluate(()=>window.avatarRingCount),0);await page.screenshot({path:'.local/229-no-ring-portrait.png'});
 console.log('PASS: LV2-5 world avatar ring overlays absent; LV5 portrait ring absent');
