...
try{
 ...
 await one.locator('#department-work-close').click();await document(one);assert.equal(await one.locator('#department-text').inputValue(),'책을 종류별로 정리하고 친구에게 책을 소개했어요.');check('문서 E 상호작용·작성·저장·닫았다 다시 열기');
 ...
 await teacher.locator('#department-award-amount').fill('12');await teacher.locator('#department-award').click();await teacher.locator('#department-balance').filter({hasText:'12개'}).waitFor();await one.waitForFunction(()=>document.getElementById('department-text').value==='');assert.equal(planet.work.report.status,'draft');assert.equal(p1.starShards,0);assert.equal(p2.starShards,0);check('교사 지급은 부서 잔액에만 반영·실적 백지 초기화·학생 창 실시간 갱신');
