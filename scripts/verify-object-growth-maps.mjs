...
try{
 ...
 const prompt=page.locator('#interact-prompt');await prompt.filter({hasText:'F 상호작용하기'}).waitFor();await page.locator('#interact-object').filter({hasText:pillar.name}).waitFor();
 ...
 await page.locator('#world').focus();await page.keyboard.press('f');await page.locator('#temple-dialog').waitFor({state:'visible'});await prompt.waitFor({state:'hidden'});await page.locator('#temple-close').click();check('F 키 상호작용 유지·창이 열리면 안내 숨김');
