...
const start=async name=>{await page.getByRole('button',{name,exact:true}).click();await page.getByRole('button',{name:/^(시작|다시 시작|처음부터 다시)$/}).click();};
...
try{
  ...
  for(const [name,rows,columns] of [['하',3,4],['중',4,6],['상',6,6]]){
  ...
  await start('하');const closeToastCount=await page.evaluate(()=>window.toasts.length);await page.locator('#arcade-close').click();await page.clock.fastForward('01:01');
