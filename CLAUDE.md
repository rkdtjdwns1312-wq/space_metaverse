...
### 서브에이전트는 파이프라인을 다시 적용하지 않는다

- 이 파이프라인은 **본체(메인 세션)만** 운영한다. Agent 도구로 띄워진 서브에이전트(Sonnet·Opus)는 자신이 한 단계의 실행자이므로, 이 파일을 읽었더라도 다시 Sonnet/Opus 하위 에이전트를 띄우거나 Opus 검증을 스스로 수행하지 않고 맡은 일을 직접 끝낸다. (2026-09-12 통합 에이전트가 파이프라인을 재귀 적용해 시간이 낭비된 사례에서 추가.)
- 본체는 서브에이전트 프롬프트에 "당신은 파이프라인의 한 단계 실행자이며 재위임하지 않는다"를 명시한다.
...
- 테스트: `node --test tests/*.test.js`(단위·실시간), `node scripts/verify-browser.mjs`(설치된 Edge 헤드리스로 교사·학생 창 실제 조작). 코드 변경 후 둘 다 통과해야 한다. `교실-시작.cmd`는 CRLF 개행을 유지한다(`.gitattributes`로 고정).
- 진행 상태와 미완료 사항은 `메인.md`의 '현재 상태와 미완료 사항' 절을 기준으로 한다. 최신 인계 지시서: `기록/2026-09-13_09-42-00_Codex_인수인계_지시서.md`(환경·구조·설계 원칙·프로토콜·다음 작업·함정. Claude Code가 다시 이어받을 때도 먼저 읽는다).
- 2026-09-13부터 사용자는 Codex로 작업을 시작한다. Claude Code가 다시 작업할 때는 `git pull` 후 Codex가 남긴 최신 기록을 먼저 읽는다.
- 테스트 현황: 단위 81개, 브라우저 검증 82건(약 120~130초). 새 단계마다 검사를 추가한다. 사용자가 화면에서 확인하기 전에는 다음 STEP 기능을 시작하지 않는다.
