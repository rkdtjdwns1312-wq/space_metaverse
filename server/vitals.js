...
export function ensureVitals(player){
  const limits=vitalsOf(player?.avatar?.level);if(!limits)return null;
  let value=player.battleVitals;
  if(!value||value.level!==player.avatar.level){
    value=player.battleVitals={level:player.avatar.level,hp:limits.hp.max,mp:limits.mp.max,defeatedAt:null};
  }
  return value;
}
export function playerVitals(player){
  const value=ensureVitals(player),limits=vitalsOf(player?.avatar?.level);if(!value)return null;
...
export function damagePlayer(player,power,now){
  ...
  const damage=damageAfterDefense(power,defensePowerOf(player.avatar.level,player.avatar.constellationId));
